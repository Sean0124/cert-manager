...
import (
	...
	cmutil "github.com/Sean0124/cert-manager/pkg/util"
	"github.com/Sean0124/cert-manager/test/e2e/framework"
	"github.com/Sean0124/cert-manager/test/e2e/framework/addon"
	"github.com/Sean0124/cert-manager/test/e2e/framework/addon/pebble"
	"github.com/Sean0124/cert-manager/test/e2e/framework/addon/tiller"
	"github.com/Sean0124/cert-manager/test/e2e/framework/log"
	. "github.com/Sean0124/cert-manager/test/e2e/framework/matcher"
	"github.com/Sean0124/cert-manager/test/e2e/util"
	"github.com/Sean0124/cert-manager/test/unit/gen"
	"github.com/Sean0124/cert-manager/test/util/generate"
)
...
var _ = framework.CertManagerDescribe("ACME Certificate (HTTP01)", func() {
	...
	It("should obtain a signed certificate with a single CN from the ACME server", func() {
		...
		cert := generate.Certificate(generate.CertificateConfig{
			...
			DNSNames:   []string{acmeIngressDomain},
		})
	...
	It("should obtain a signed certificate with a CN and single subdomain as dns name from the ACME server", func() {
		...
		cert := generate.Certificate(generate.CertificateConfig{
			...
			SecretName: certificateSecretName,
			IssuerName: issuerName,
			DNSNames:   []string{fmt.Sprintf("%s.%s", cmutil.RandStringRunes(5), acmeIngressDomain)},
		})
		_, err := certClient.Create(cert)
		Expect(err).NotTo(HaveOccurred())
		By("Verifying the Certificate is valid")
